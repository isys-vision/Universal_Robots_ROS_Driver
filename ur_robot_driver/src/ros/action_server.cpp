...
{
  return
//          validateState(gh, res) &&
          validateJoints(gh, res)  &&
          validateTrajectory(gh, res);
}
...
void ActionServer::trajectoryThread()
{
  ...
  while (running_)
  {
    ...
    {
      LOG_ERROR("Trajectory without t0 recieved");
      //trajectory.push_back(TrajectoryPoint(q_actual_, qd_actual_, std::chrono::microseconds(0)));
      res.error_code = -100;
      res.error_string = "Trajectory without t0 recieved";
      curr_gh_.setAborted(res, res.error_string);
    }
    ...
    LOG_INFO("Attempting to start follower");
    if (follower_->start())
    {
